...
/**
 * @description Add clyde product to cart
 */
function clydeAddProductToCart() {
    var $form = $('.pdpForm');
    if (ClydeSitePreferences.CLYDE_WIDGET_ENABLED) {
    ...
    addItemToCart($form).then(function (response) {
        ...
        if ($uuid.length > 0 && $uuid.val().length > 0) {
            ...
            if (!$('.add-to-cart').hasClass('sub-product-item')) {
                dialog.close();
            }
 ...
 var addToCart = function (e) {
    ...
    if (ClydeSitePreferences.CLYDE_WIDGET_ENABLED) {
        ...
        if (clydeSettings.productPage == true) {
            if (!selectedContract && clydeSettings.modal == true && Clyde.getActiveProduct().contracts && Clyde.getActiveProduct().contracts.length > 0) {
